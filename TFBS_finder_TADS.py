...
def main():
    ...
    path = "./data/pwm/"
    directory = "TADS/" + input_fasta.name[13:]
    os.mkdir("TFBS_output/{}".format(directory))
    
    for tf in glob.glob(os.path.join(path, "*.pwm")):
        name = tf.split("/")[-1].replace(".pwm","")
        threshold = thresholds[name]
        subprocess.call(["java", "-cp", "sarus-26Nov2017.jar", "ru.autosome.SARUS", input_fasta.name, tf, threshold, "--output-bed", "skipn"], stdout=open("TFBS_output/TADS/{}/{}.bed".format(input_fasta.name[13:],name), "w"))
